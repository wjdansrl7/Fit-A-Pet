...
const useHealthDataStore = create((set, get) => ({
  ...
  checkQuestCompletion: async () => {
    ...
    const walkQuests = personalQuest.filter(
      (quest) => quest.category === 'WALK'
    );
    const sleepQuests = personalQuest.filter(
  ...
  //     console.log(
  //       `${questType} 퀘스트 ID ${questId} 완료 전송 성공:`,
  //       response.data
  //     );
  //   } catch (error) {
  //     console.error(
  ...
  //         const quest = personalQuest.find((quest) => quest.id === id);
