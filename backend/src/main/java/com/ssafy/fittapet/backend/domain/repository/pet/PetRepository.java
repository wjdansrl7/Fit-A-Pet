...
public interface PetRepository extends JpaRepository<Pet, Long> {
    ...
    @Query("SELECT p FROM Pet p WHERE p.petType = :petType AND p.petStatus = :petStatus AND p.id = :petId+1")
    Optional<Pet> findNextEvolution(PetType petType, PetStatus petStatus, Long petId);
